...
assert.throws(() => new RemoteNodeClient({ machineId: "m", url: "http://localhost", accessClientId: "i", accessClientSecret: "s", nodeToken: "t" }), /HTTPS/);
...
const client = new RemoteNodeClient({ machineId: "m", url: "https://remote.test", accessClientId: "i", accessClientSecret: "s", nodeToken: "t", maxBodyBytes: 100 });
...
const result = await new RemoteNodeClient({ machineId: "m", url: "https://remote.test", accessClientId: "i", accessClientSecret: "s", nodeToken: "t", maxBodyBytes: 1000 }).execute("read_file", { workspaceId: "w", path: "x" }, { requestId: "r", signal: new AbortController().signal });
...
const identityClient = new RemoteNodeClient({ machineId: "m", url: "https://remote.test", accessClientId: "i", accessClientSecret: "s", nodeToken: "t", maxBodyBytes: 1000 });
...
await assert.rejects(
  new RemoteNodeClient({ machineId: "m", url: "https://remote.test", accessClientId: "i", accessClientSecret: "s", nodeToken: "t", timeoutMs: 5 }).hello(new AbortController().signal),
...
await assert.rejects(
  new RemoteNodeClient({ machineId: "m", url: "https://remote.test", accessClientId: "i", accessClientSecret: "s", nodeToken: "t" }).hello(new AbortController().signal),
...
await assert.rejects(
  new RemoteNodeClient({ machineId: "m", url: "https://remote.test", accessClientId: "i", accessClientSecret: "s", nodeToken: "t" }).execute("read_file", { workspaceId: "w", path: "x" }, { requestId: "access", signal: new AbortController().signal }),
  (error: unknown) => error instanceof Error && error.name === "TargetUnavailableError" && !error.message.includes("Cloudflare"),
);
